...
ifeq ($(TARGET_VENDOR_SHOW_MAINTENANCE_VERSION),true)
    SOMEFEAKOS_VERSION_MAINTENANCE := $(PRODUCT_VERSION_MAINTENANCE)
else
    SOMEFEAKOS_VERSION_MAINTENANCE := 0
endif
...
ifeq ($(LINEAGE_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        SOMEFEAKOS_VERSION := $(PRODUCT_ROM_NAME)-$(PRODUCT_VERSION_MAJOR)$(LINEAGE_BUILD)
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            ifeq ($(LINEAGE_VERSION_MAINTENANCE),0)
                SOMEFEAKOS_VERSION := $(PRODUCT_ROM_NAME)-$(PRODUCT_VERSION_MAJOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(LINEAGE_BUILD)
            else
                SOMEFEAKOS_VERSION := $(PRODUCT_ROM_NAME)-$(PRODUCT_VERSION_MAJOR).$(LINEAGE_VERSION_MAINTENANCE)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(LINEAGE_BUILD)
            endif
        else
            SOMEFEAKOS_VERSION := $(PRODUCT_ROM_NAME)-$(PRODUCT_VERSION_MAJOR)$(LINEAGE_BUILD)
        endif
    ...
    ifeq ($(LINEAGE_VERSION_MAINTENANCE),0)
        ifeq ($(LINEAGE_VERSION_APPEND_TIME_OF_DAY),true)
            SOMEFEAKOS_VERSION := $(PRODUCT_ROM_NAME)-$(PRODUCT_VERSION_MAJOR)-$(shell date -u +%Y%m%d_%H%M%S)-$(LINEAGE_BUILDTYPE)$(LINEAGE_EXTRAVERSION)-$(LINEAGE_BUILD)
        else
...
ifneq ($(PRODUCT_DEFAULT_DEV_CERTIFICATE),build/target/product/security/testkey)
    ifneq ($(LINEAGE_BUILDTYPE), UNOFFICIAL)
        ...
        ifeq ($(LINEAGE_VERSION_MAINTENANCE),0)
            ...
            LINEAGE_DISPLAY_VERSION := $(PRODUCT_VERSION_MAJOR).$(LINEAGE_VERSION_MAINTENANCE)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(LINEAGE_BUILD)
        endif
    endif
endif
endif
