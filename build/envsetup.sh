...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/somefeakos/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch somefeakos_$target-$variant
        fi
    fi
    return $?
}
...
function repopick() {
    T=$(gettop)
    $T/vendor/somefeakos/build/tools/repopick.py $@
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/somefeakos/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
